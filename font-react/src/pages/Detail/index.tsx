...
function Detail() {
  ...
  const {
    ...
    queryKey: ["phoneNumberDetail", number],
    queryFn: async () => {
      if (!number) throw new Error("Số điện thoại không hợp lệ");
      const result = await DeatilPhoneNumber(number);
      if (result.status === 200) {
        return result.data.data as DetailITF;
      }
      throw new Error("Không thể tải thông tin chi tiết số điện thoại");
    },
    staleTime: 5 * 60 * 1000,
    retry: 3,
    enabled: !!number,
  ...
  return (
    <div className={cx("wrapper")}>
      <div className="m-1">
        ...
        <hr className="my-2" />
        {/* Rest of your UI code remains the same, just remove the conditional rendering since we handle loading/error states above */}
        ...
        <div className="p-3 border rounded-xl shadow-md shadow-gray-300 mb-2">
          <form action="" method="post">
            ...
            <button
              className={`${cx("btn-submit")} w-full p-3 rounded-xl text-white`}
            >
              <div>
                <b>Đặt mua</b>
        ...
        <div className="p-3 border rounded-xl shadow-md shadow-gray-300 mb-2">
          ...
          <div className="mb-1">
            <b> Cách 2:</b>{" "}
            <p>
              Quý khách đến cửa hàng SIMvn tại Hà Nội, Tp.HCM, Đà Nẵng, Hưng Yên
              làm thủ tục (Xem danh sách cửa hàng ở chân trang)
            </p>
          </div>
          <div className="mb-1">
            <b> Cách 3:</b>{" "}
            <p>
              Đặt hàng trên website, gọi hotline hoặc chat với SIM.vn sau đó sẽ
              có nhân viên tiếp nhận thông tin, hồ sơ sang tên sau đó sẽ gửi COD
              hoặc giao Hỏa Tốc trong 30 phút (bạn phải hỗ trợ phí giao sim) đến
              tận nhà, nhận sim bạn kiểm tra đúng thông tin chính chủ và trả
              tiền cho bưu tá
            </p>
          </div>
          <div>
